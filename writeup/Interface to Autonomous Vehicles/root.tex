...
\title{\LARGE \bf
Designing a Hardware-Agnostic Interface Between Route and Trajectory Planning in Self-Driving Cars
}
...
%To optimize fuel efficiency, for example, a gasoline-powered vehicle must be driven differently than an electric-gas hybrid or fully electric vehicle with regenerative braking.
...
% An autonomous truck prioritizes fuel efficiency. An autonomous taxi must avoid maneuvers that would cause discomfort or alarm to customers.
...
% TODO Why are we talking about PID? Maybe cut it or shrink it to 2 sentences.
...
% Classical control has been applied to complex, multi-body vehicle models to improve in highly nonlinear situations \cite{CHEBLY201712526}. The authors also discuss the controller's robustness to errors caused by variations in the vehicle's mass or tire stiffness, verifying the controller can continue to track the reference trajectory with a $\pm30\%$ error in these parameters.
...
%In this paper we designed a trajectory planner to reduce hardware dependencies in higher-level AV route planning.
